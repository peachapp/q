...
router.beforeEach(async (to, from, next) => {
  ...
  if (token) {
    // 有token 用户已登录过
    // 先获取用户信息，看之前是否获取过用户信息
    // 注意！！这里不能使用this.$store，所以需要导入Vuex实例store
    const { userInfo } = store.state.user;
    if (userInfo && userInfo.id) {
      // 存在用户信息 放行
      next();
    } else {
      // 获取用户信息可能失败 用try...catch...
      try {
        // 不存在用户信息 使用token获取用户信息
        // 因为dispatch返回值是一个promise所以加await获取actions中的返回值
        // 获取成功
